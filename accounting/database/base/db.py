...
from sqlalchemy import select, update, Select, Column, Update
...
class DB:
    ...
    # 删除数据
    async def delete(self, table: T, *criteria: bool) -> T | List[T]:
        async with self.async_session() as session:
            async with session.begin():
                deleted_records = []
                if criteria:
    ...
    async def update(
        ...
        **values,
        ...
        if not isinstance(table_or_stmt, (Select, Select[tuple[Any]], Update)):
            table_or_stmt = (
                update(table_or_stmt)
                .where(*criteria)
                .values(**values)
                .returning(table_or_stmt)
            )
        else:
            table_or_stmt = table_or_stmt.returning(table_or_stmt.table)
        async with self.async_session() as session:
            async with session.begin():
                if result := (await session.execute(table_or_stmt)).first():
                    return result[0]
